...
function AlternatingRow({
  ...
  section: RabiaAlternatingSection;
  index: number;
}) {
  const { t } = useLang();
...
export function RabiaAlternatingSections() {
  const { t } = useLang();

  return (
    <div id="rabia-program" className="bg-[#FAF8F4]">
      <div className="px-6 pb-4 pt-8 text-center md:px-8 md:pb-6 md:pt-12">
        <p className="text-[11px] uppercase tracking-[0.42em] text-[#9A7B3A]">
          {t("مكوّنات المشروع", "Project Components")}
        </p>
        <h2 className="mt-3 font-heading text-2xl font-semibold text-[#1A1612] md:text-4xl">
          {t(
            "أبراج · مسجد · ساحات وتجارة",
            "Towers · Mosque · Plazas & Retail",
          )}
        </h2>
      </div>

      {RABIA_ALTERNATING.map((section, index) => (
