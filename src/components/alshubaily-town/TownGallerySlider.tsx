...
import {
  TOWN_GALLERY,
  TOWN_GALLERY_HEADER,
} from "@/data/alshubaily-town-content";
import { cn } from "@/lib/utils";

gsap.registerPlugin(ScrollTrigger);

const slides = TOWN_GALLERY;
...
export function TownGallerySlider() {
  ...
  return (
    <section
      ref={sectionRef}
      id="alshubaily-town-gallery"
      className="overflow-hidden bg-[#1A1410] pb-10 pt-12 md:pb-14 md:pt-16"
    ...
    >
      ...
      >
        <p className="text-xs font-medium uppercase tracking-[0.35em] text-[#C9A962]">
          {t(
            TOWN_GALLERY_HEADER.eyebrowAr,
            TOWN_GALLERY_HEADER.eyebrowEn,
          )}
        </p>
        <h2 className="mt-3 font-heading text-2xl font-bold leading-snug text-white md:text-4xl">
          {t(
            TOWN_GALLERY_HEADER.titleAr,
            TOWN_GALLERY_HEADER.titleEn,
          )}
        </h2>
        <p className="mx-auto mt-3 max-w-xl text-sm leading-relaxed text-white/65 md:text-base">
          {t(
            TOWN_GALLERY_HEADER.subtitleAr,
            TOWN_GALLERY_HEADER.subtitleEn,
          )}
        </p>
      </div>
      ...
      <div data-gallery-reveal className="relative w-full px-0">
            ...
            {slides.map((slide, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "absolute top-1/2 overflow-hidden rounded-2xl shadow-2xl transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] md:rounded-3xl",
        ...
        <div className="mt-5 flex flex-wrap items-center justify-center gap-1.5 px-4 md:mt-6">
          {slides.map((_, i) => (
            <button
              key={i}
              type="button"
