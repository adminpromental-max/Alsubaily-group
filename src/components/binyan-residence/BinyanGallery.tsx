...
import {
  BINYAN_GALLERY,
  BINYAN_GALLERY_HEADER,
} from "@/data/binyan-residence-content";
import { cn } from "@/lib/utils";

gsap.registerPlugin(ScrollTrigger);

const slides = BINYAN_GALLERY;
...
export function BinyanGallery() {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative overflow-hidden bg-[#F8F5F0] py-16 md:py-24"
      ...
      <div className="mx-auto max-w-6xl px-6 md:px-8">
        <div data-gallery-reveal className="mb-10 text-center md:mb-12">
          <p className="text-xs font-medium uppercase tracking-[0.35em] text-[#C4783A]">
            {t(
              BINYAN_GALLERY_HEADER.eyebrowAr,
              BINYAN_GALLERY_HEADER.eyebrowEn,
            )}
          </p>
          <h2 className="mt-2 font-heading text-3xl font-bold text-[#2A3340] md:text-4xl">
            {t(
              BINYAN_GALLERY_HEADER.titleAr,
              BINYAN_GALLERY_HEADER.titleEn,
            )}
          </h2>
          <p className="mx-auto mt-3 max-w-xl text-sm text-[#2A3340]/65">
            {t(
              BINYAN_GALLERY_HEADER.subtitleAr,
              BINYAN_GALLERY_HEADER.subtitleEn,
            )}
          </p>
        </div>
              ...
              >
                <div
                  className={cn(
                    ...
                    isActive && "ring-2 ring-[#C4783A]/40",
                  )}
                >
                  <img
                    src={slide.src}
                  ...
                  />
                  <div className="bg-[#1E2430] px-5 py-4 text-center">
                    <h3 className="font-heading text-lg font-bold text-white">
                      {t(slide.titleAr, slide.titleEn)}
                    </h3>
                    <p className="mt-1 text-sm text-[#F0D4B8]">
        ...
        >
          <button
            ...
            className="flex h-11 w-11 items-center justify-center rounded-full border border-[#3D4F63]/20 bg-white text-[#3D4F63] transition hover:bg-[#3D4F63] hover:text-white"
          >
            <ChevronRight
            ...
            {slides.map((_, i) => (
              <button
                ...
                className={cn(
                  ...
                  active === i
                    ? "w-8 bg-[#3D4F63]"
                    : "w-2 bg-[#3D4F63]/25 hover:bg-[#3D4F63]/50",
                )}
              />
            ))}
          </div>
