...
import { Cpu, Leaf, Network } from "lucide-react";
import { useLang } from "@/contexts/lang-context";
import { BINYAN_PILLARS, BINYAN_VISION } from "@/data/binyan-residence-content";
import { cn } from "@/lib/utils";

gsap.registerPlugin(ScrollTrigger);

const PILLAR_ICONS = [Cpu, Network, Leaf] as const;

export function BinyanVision() {
    ...
    <section ref={sectionRef} className="bg-[#FAF8F4] py-16 md:py-24">
      <div className="mx-auto max-w-6xl px-6 md:px-8">
        <div data-pillar-reveal className="mb-10 text-center md:mb-12">
          <p className="text-xs font-medium uppercase tracking-[0.35em] text-[#3D4F63]">
            {t(BINYAN_VISION.titleAr, BINYAN_VISION.titleEn)}
          </p>
          <p className="mx-auto mt-4 max-w-3xl text-sm leading-8 text-[#2A3340]/75 md:text-base">
            {t(BINYAN_VISION.bodyAr, BINYAN_VISION.bodyEn)}
          </p>
        </div>
          ...
          {BINYAN_PILLARS.map((pillar, i) => {
            ...
            return (
              <div
                ...
                onClick={() => setActive(i)}
                className={cn(
                  "relative cursor-pointer overflow-hidden rounded-3xl outline-none transition-[flex] duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] focus-visible:ring-2 focus-visible:ring-[#C4783A]",
              ...
              >
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-[#1E2430]/90 via-[#3D4F63]/35 to-transparent" />
                    ...
                    <h3
                      className={cn(
                        ...
                        isActive
                          ...
                          : lang === "ar"
                            ? "text-lg [writing-mode:vertical-rl]"
                            : "text-lg -rotate-90 origin-bottom-left translate-x-6",
                      )}
                    >
